...
function Band({
  ...
  const cardMap = useMemo(() => {
    ...
    // === FRONT CARD ===
    const frontX = FRONT_UV_RECT.x * W;
    const frontY = FRONT_UV_RECT.y * H;
    const frontW = FRONT_UV_RECT.w * W;
    const frontH = FRONT_UV_RECT.h * H;

    const padding = frontW * 0.07;

    // --- Orange border ---
    ctx.strokeStyle = '#c85314';
    ctx.lineWidth = Math.max(2, frontW * 0.012);
    ...
    if (backImage && backTex.image) {
      ...
      ctx.fillRect(
        logoX - frontW * 0.015,
        logoY - frontH * 0.015,
        logoSize + frontW * 0.03,
        logoSize + frontH * 0.03
      );
    ...
    // --- Role ---
    const roleX = frontX + frontW * 0.14;
    const roleY = frontY + frontH * 0.14;

    ctx.fillStyle = '#ffffff';
    ctx.font = `400 ${frontW * 0.075}px Space Grotesk, sans-serif`;
    ctx.textAlign = 'left';
    ctx.textBaseline = 'top';

    ctx.fillText('Web Developer', roleX, roleY);
    ...
    // --- Orange photo background ----
    const photoX = frontX + frontW * 0.12;
    const photoY = frontY + frontH * 0.46;
    const photoW = frontW * 0.76;
    const photoH = frontH * 0.36;

    ctx.fillStyle = '#c85314';
    ...
    if (frontImage && frontTex.image) {
      ...
      const scale = Math.max(
        photoW / frontImg.width,
        imageH / frontImg.height
      );
    ...
    // --- Name ---
    const nameX = frontX + frontW * 0.18;
    const nameY = frontY + frontH * 0.850;

    // Nama
    ctx.fillStyle = '#ffffff';
    ctx.font = `700 ${frontW * 0.075}px Arial, sans-serif`;
    ctx.textAlign = 'left';
    ctx.textBaseline = 'top';
    ...
    // Back remains the original full logo.
    if (backImage && backTex.image) {
      drawContain(
        backTex.image,
        BACK_UV_RECT.x * W,
